...
def upload(request):
	...
	#    cv_summary1=request.POST.items()
	#    return render(request, 'show.htm', {'cv_summary':cv_summary, 'cv_summary1':cv_summary1})
		...
		for job in root.iter('job'):
			if(job.attrib.get('id') == key):
				all_vacants_info.append({
					...
					'job_name': str(job.find('name').text),
					...
					.replace("br","").replace("strong","").replace("strong"," ").replace("ol"," "),
